...
어떤 블럭에 빗물이 차려면 양 옆에 해당 블록보다 높은 블록이 있어야 한다.
따라서 블록 리스트를 좌,우로 순회하면서 해당 블록의 왼쪽 블록들에서의 최댓값, 오른쪽 블록들에서의 최댓값을 블록 별로 저장한다
다시 블록 리스트를 돌면서 좌우의 최댓값이 자신보다 크면 전체 고인 빗물 크기에 좌우 최댓값 중 작은 값 본인의 차이만큼을 더한다.
